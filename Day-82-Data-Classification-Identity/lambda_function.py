...
TRANSACTIONS_TABLE_NAME = 'FinanceAgent-Transactions'
...
def fetch_fresh_data(user_id, profile):
    ...
    if data_source == 'wise':
        ...
        return ingest_plaid_data(user_id), 'plaid'


def user_has_persisted_data(user_id):
    """True si DynamoDB ya tiene perfil o transacciones para ese user_id."""
    if not user_id:
        return False
    try:
        response = dynamodb.Table(TRANSACTIONS_TABLE_NAME).query(
        ...
        logger.warning("Could not verify persisted user data", extra={"user_id": user_id, "details": str(e)})
        return False


def resolve_user_identity(payload, user_email_jwt, default_user_id, default_user_name):
    """
    Cognito `sub` es el identificador estable. El email puede cambiar o variar entre
    login manual/social, y estaba separando el dashboard de los datos sincronizados.
    """
    cognito_sub = payload.get('sub') or default_user_id
    ...
    if user_email_jwt and user_email_jwt.lower() == 'ericrodriguezpacheco@outlook.com':
        resolved_id = cognito_sub
    elif user_has_persisted_data(cognito_sub):
        resolved_id = cognito_sub
    elif user_email_jwt and user_has_persisted_data(user_email_jwt.lower()):
        resolved_id = user_email_jwt.lower()
    elif user_email_jwt and '@' in cognito_user and not cognito_sub:
        resolved_id = user_email_jwt.lower()
    else:
        resolved_id = cognito_sub

    if user_email_jwt:
        resolved_name = real_name if real_name else user_email_jwt.split('@')[0].capitalize()
    else:
        resolved_name = default_user_name

    return resolved_id, resolved_name


def acquire_sync_lock(user_id, reason, window_seconds):
    """
    Limita llamadas manuales/repetidas a Plaid/Wise. Evita que `sync=true` pueda
    disparar gasto por abuso y mantiene un refresh automático barato.
    """
    now_ts = int(time.time())
    bucket = now_ts // window_seconds
    lock_key = f"source_sync#{reason}#{user_id}#{bucket}"
    try:
        ...
        logger.warning("Could not acquire sync lock", extra={"user_id": user_id, "details": str(e)})
        return False


def persist_source_sync(raw_data, source, user_id, profile):
    if source == 'wise':
        save_wise_to_dynamo(raw_data, user_id)
    else:
        save_to_dynamo(raw_data, user_id)

    now = datetime.datetime.now(datetime.timezone.utc)
    profile['last_source_sync_at'] = now.isoformat()
    profile['last_source_sync_date'] = now.strftime('%Y-%m-%d')
    save_user_profile(profile)
    return get_transactions_from_dynamo(user_id)


def sync_source_if_allowed(user_id, profile, reason='auto_daily', force=False):
    today = datetime.datetime.now(datetime.timezone.utc).strftime('%Y-%m-%d')
    already_synced_today = profile.get('last_source_sync_date') == today

    if profile.get('data_source', 'plaid') == 'wise' and reason in ('auto_daily', 'webhook', 'manual'):
        logger.info("Skipping reactive sync for Wise profile", extra={"user_id": user_id, "reason": reason})
        return None, False

    if reason == 'auto_daily' and already_synced_today:
        return None, False

    window_seconds = 900 if force else 86400
    if not acquire_sync_lock(user_id, reason, window_seconds):
        logger.info("Source sync skipped by lock", extra={"user_id": user_id, "reason": reason})
        return None, False

    raw_data, source = fetch_fresh_data(user_id, profile)
    return persist_source_sync(raw_data, source, user_id, profile), True
    ...
    try:
        ...
        if 'Records' in event:
            for record in event['Records']:
                ...
                if task == 'sync_transactions':
                    profile = get_user_profile(user_id, DEFAULT_USER_NAME)
                    sync_source_if_allowed(user_id, profile, 'webhook', force=True)
                    continue
                ...
                if task in ['daily_report', 'hourly_alert_check']:
                    ...
                    if task == 'daily_report':
                        ...
                        raw_data, source = fetch_fresh_data(user_id, profile)
                        saved_items = persist_source_sync(raw_data, source, user_id, profile)
                    else:
                    ...
                    for t in daily_items:
                        ...
                        if is_expense_tx(desc, amt, t):
                            total_daily_spent += amt

                    # Alerta por gasto alto
                    is_alert = total_daily_spent > 100.0
                    if is_alert:
                        send_sms_if_needed(total_daily_spent, user_id, profile)
                    ...
                    if task == 'daily_report':
                        ...
                        current_streak, _, _ = update_user_streak(profile, total_daily_spent, total_income)
        ...
        # ──────────────────────────────────────────────────────────────────────
        # MODO 2B: PLAID WEBHOOK (sync reactiva sin botón en el dashboard)
        # ──────────────────────────────────────────────────────────────────────
        if query_params.get('action') == 'plaid_webhook':
            try:
                ...
                if webhook_type == 'TRANSACTIONS' and webhook_code in sync_codes:
                    sqs.send_message(
                        QueueUrl=SQS_QUEUE_URL,
                        MessageBody=json.dumps({"task": "sync_transactions", "user_id": DEFAULT_USER_ID})
                    )
                    logger.info("Plaid webhook queued transaction sync", extra={"webhook_code": webhook_code})
                ...
                logger.error("Plaid webhook failed", extra={"details": str(e)})
        ...
        if auth_header.startswith('Bearer '):
            try:
                ...
                user_id, user_name = resolve_user_identity(
                    payload, user_email_jwt, DEFAULT_USER_ID, DEFAULT_USER_NAME
                )
        ...
        if query_params.get('action') == 'save_preferences':
            ...
            # FIX 1: Evitar sobrescribir el nombre con un string vacío del frontend
            ...
            if 'alert_phone_number' in body:
                user_profile['alert_phone_number'] = body.get('alert_phone_number', '').strip()
            ...
            if not has_received_welcome and user_profile.get('wants_daily_email', True):
                # FIX 2: Extraer correo de forma infalible desde el token JWT
                dest_email = user_email_jwt if user_email_jwt else get_report_email(user_profile)
                
                welcome_analysis = (
                    f"Welcome to the system, {user_profile.get('display_name', display_name)}. "
                ...
                try:
                    ses.send_email(
                        ...
                        Message={
                            # FIX 4: Asunto serio, cero emojis.
            ...
            # FIX: Si el usuario está cambiando sus preferencias, invalidamos su caché de IA
            # para que el siguiente mensaje use su nuevo nombre/objetivo.
            try:
                # Generamos la llave actual (la vieja) y la borramos
                old_cache_key = generate_cache_key(saved_items, total_income, total_expenses, f"dashboard_{ai_tone}", user_id)
                cache_table.delete_item(Key={'cache_key': old_cache_key})
            except Exception:
        ...
        # ── DELETE: Botón Nuclear (Borrar Cuenta Definitivo) ──────────────────────────────
        if query_params.get('action') == 'delete_account':
            # 1. Limpieza total de DynamoDB (Transacciones y Perfil)
            ...
            # Limpiar memoria residual y caché si existen
            try:
                # Borramos cachés asociadas a este usuario
                cache_resp = cache_table.scan(FilterExpression=boto3.dynamodb.conditions.Attr('user_id').eq(user_id))
                with cache_table.batch_writer() as batch:
                    for item in cache_resp.get('Items', []):
                        batch.delete_item(Key={'cache_key': item['cache_key']})
            except Exception as e:
                logger.warning("Fallo al limpiar cache", extra={"details": str(e)})
            
            # 2. Borrado de Identidad en Cognito (Búsqueda Infalible)
            try:
                # El JWT de Cognito SIEMPRE contiene el Username exacto bajo 'cognito:username'
                # Esto es infalible tanto para cuentas con contraseña como para cuentas de Google.
                ...
                if cognito_username:
                    ...
                    logger.info(f"Usuario {cognito_username} fulminado de Cognito.")
                else:
                    logger.warning("No se encontró cognito:username en el payload.")
                        
            except Exception as e:
                logger.error("Error fulminando de Cognito", extra={"details": str(e)})
            ...
            return {
                ...
                'body': json.dumps({
                    ...
                    "data": {
                        ...
                        "alert_phone_number": user_profile.get('alert_phone_number', ''),
        ...
        # SMART SYNC: una vez al día por usuario. No depende del botón ni de
        # llamadas manuales ilimitadas; el candado en cache_table evita abuso.
        current_month = datetime.datetime.now().strftime('%Y-%m')
        has_current_month_data = any(t.get('transaction_date', '').startswith(current_month) for t in saved_items)
        
        needs_sync = not saved_items or not has_current_month_data or user_profile.get('last_source_sync_date') != datetime.datetime.now(datetime.timezone.utc).strftime('%Y-%m-%d')

        if needs_sync:
            logger.info("🔄 Sincronización automática controlada...")
            synced_items, did_sync = sync_source_if_allowed(user_id, user_profile, 'auto_daily')
            if did_sync and synced_items is not None:
                saved_items = synced_items
        elif force_sync:
            synced_items, did_sync = sync_source_if_allowed(user_id, user_profile, 'manual', force=True)
            if did_sync and synced_items is not None:
                saved_items = synced_items
        ...
        for t in saved_items:
            ...
            if acct_m == current_month_str:
                if income: total_income   += abs(amount)
                elif is_expense_tx(desc, amount, t):
                    total_expenses += abs(amount)
            elif acct_m == previous_month_str:
                if income: last_month_income += abs(amount)

        projected_monthly_income = total_income
        ...
        target_month_filter = query_params.get('month', current_month_str)
        
        # NUEVO: Sumar los gastos del mes exacto que el usuario está consultando
        target_month_expenses = 0.0
        for t in saved_items:
            amt = float(t.get('amount', 0))
            desc = t.get('description', '')
            t_date = t.get('transaction_date', '').split('T')[0].split('#')[0]
            
            if get_accounting_month(t_date, is_income_tx(desc, amt)) == target_month_filter:
                if is_expense_tx(desc, amt, t):
                    target_month_expenses += abs(amt)

        # Ahora sí, la proyección usa los gastos correctos y limpios
        projected_spend = calculate_projection(target_month_expenses, target_month_filter)
