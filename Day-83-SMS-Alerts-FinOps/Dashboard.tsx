...
const isExpenseTransaction = (tx: Transaction) => {
  ...
  return amount > 0 && !isIncomeTransaction(tx) && !isInternalTransfer(tx);
};
  ...
  useEffect(() => {
    ...
    filteredTxs.forEach((tx: any) => {
      ...
      if (incomeTransaction) {
        ...
        if (desc.includes('rent') || desc.includes('mortgage') || desc.includes('housing') || desc.includes('alquiler')) cat = "Housing";
  ...
  return (
    <div className="min-h-screen bg-gray-50 text-gray-800 font-sans selection:bg-green-200 pb-20" style={{ fontFamily: "'DM Sans', 'Inter', system-ui, sans-serif" }}>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
        ...
        {!isFirstTimeSetup && (
          currentView === 'profile' ? (
            <Profile avatarUrl={prefAvatar} isSaving={isSavingPrefs} onSave={savePreferences} onSignOut={handleSignOut} prefName={prefName} prefSavingsGoal={prefSavingsGoal} prefTone={prefTone} prefWantsEmail={prefWantsEmail} prefAlertPhone={prefAlertPhone} setAvatarUrl={setPrefAvatar} setPrefName={setPrefName} setPrefSavingsGoal={setPrefSavingsGoal} setPrefTone={setPrefTone} setPrefWantsEmail={setPrefWantsEmail} setPrefAlertPhone={setPrefAlertPhone}/>
          ) : (
            <>
              ...
              <div className="grid grid-cols-1 md:grid-cols-12 gap-5 items-stretch">
                ...
                <div className="md:col-span-7 flex flex-col gap-5">
                  ...
                  <SpendingOverview loading={loading} pieData={pieData} totalExpenses={totalExpenses} totalIncome={totalIncome} />
                </div>
              </div>
            </>
          )
