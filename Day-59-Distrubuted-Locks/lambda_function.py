    ...
    try:
            ...
            for record in event['Records']:
                ...
                if task in ['daily_report', 'hourly_alert_check']:
                    ...
                    if task == 'daily_report':
                        ...
                        # Para alertas horarias: leer de DynamoDB (rápido, sin riesgo de timeout)
                        saved_items = get_transactions_from_dynamo(user_id)
                        if not saved_items:
                            # Solo llamamos a Plaid como fallback si DynamoDB está vacío
                            saved_items = save_to_dynamo(ingest_plaid_data(), user_id)
        ...
        if is_scheduled_event or test_email:
            try:
                ...
                # 11 UTC = 13:00 en España (Verano CEST, UTC+2)
                ...
                if test_email:
                    ...
                    # FIX 2: EVITAR EL 'SCAN' PAGINADO DE DYNAMODB PARA TU CUENTA. 
                    # Hacemos un GET directo (O(1)) garantizado a tu perfil para que nunca quedes oculto.
                    ...
                    # Escaneo seguro para futuros usuarios
                    try:
