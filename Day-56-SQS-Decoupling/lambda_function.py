...
def lambda_handler(event, context):
    try:
        ...
        if 'Records' in event:
            for record in event['Records']:
                ...
                if task == 'daily_report':
                    # ¡AQUÍ ESTÁ LA MAGIA DEL NOMBRE! Leemos de BD para saber que eres "Erich"
                    ...
                    # Llamada pesada a IA (is_report_time=True) pasándole el display_name correcto ("Erich")
        ...
        if is_scheduled_event or test_email:
            try:
                # En lugar de bloquearse procesando, escanea la BD y lanza los trabajos a SQS al instante
                response = table.scan(FilterExpression=Key('transaction_id').eq('METADATA'))
                users = response.get('Items', [])
                if not users and test_email: users = [{'user_id': DEFAULT_USER_ID}] # Fallback
                
                for u in users:
                    sqs.send_message(QueueUrl=SQS_QUEUE_URL, MessageBody=json.dumps({"task": "daily_report", "user_id": u.get('user_id')}))
                
                logger.info(f"Fan-out complete. Queued {len(users)} reports in SQS.")
                return {'statusCode': 200, 'headers': {"Content-Type": "application/json"}, 'body': json.dumps(f"Reports queued successfully for {len(users)} users.")}
            except Exception as e:
        ...
        if auth_header.startswith('Bearer '):
            try:
                ...
                logger.info("🔐 Authenticated User Detected", extra={"user_id": user_id, "user_email": user_email})
            except Exception as e:
                logger.error("Error decoding JWT", extra={"details": str(e)})

        logger.append_keys(user_id=user_id)
        
        # --- NUEVO: RECUPERAR PERFIL DE USUARIO ---
        ...
        logger.info("Execution started", extra={"display_name": display_name, "ai_tone": ai_tone})

        # --- NUEVO: ENRUTAMIENTO HTTP PARA PREFERENCIAS ---
        http_method = event.get('requestContext', {}).get('http', {}).get('method', 'GET')
        query_params = event.get('queryStringParameters') or {}
        
        # RUTA POST: Guardar Preferencias (SIN CORS MANUAL)
        if http_method == 'POST' or query_params.get('action') == 'save_preferences':
            ...
            # --- FIX: BOTO3 REQUIERE EL TIPO 'DECIMAL', NO FLOAT ---
        ...
        # RUTA GET: Obtener Preferencias (SIN CORS MANUAL)
        ...
        is_report_time = event.get('source') == 'aws.events' or (datetime.datetime.now().hour == 7) or test_email
        ...
        for t in saved_items:
            ...
            elif t_date.startswith(previous_month_str):
                if is_income: last_month_income += abs(amount)

        # Si estamos a principio de mes y no has cobrado, usamos el histórico del mes pasado
        projected_monthly_income = total_income if total_income > 0 else last_month_income
        if projected_monthly_income == 0: projected_monthly_income = 1500.00 # Fallback extremo
        ...
        # Le pasamos el ingreso proyectado al motor de rachas
        if is_report_time:
            current_streak, highest_streak, _ = update_user_streak(user_profile, total_daily_spent, projected_monthly_income)
        else:
            current_streak = int(user_profile.get('current_streak', 0))

        # --- CASH ACCOUNTING: El Score se basa en dinero REAL, no en promesas ---
        ...
        # Las ofertas sí usan el proyectado para saber tu poder adquisitivo real a futuro
        ...
        if not dash_message:
            ...
            # --- LA IA RECIBE TU DINERO REAL HOY PERO NO REDACTA EL CORREO (False) ---
            if user_query:
                dash_message, email_html_body = invoke_nova_ai(daily_items, total_daily_spent, total_income, total_expenses, user_query, display_name, current_streak, historical_memory, ai_tone, False)
            else:
                dash_message, email_html_body = invoke_nova_ai(daily_items, total_daily_spent, total_income, total_expenses, None, display_name, current_streak, historical_memory, ai_tone, False)
                save_to_cache(generate_cache_key(saved_items, total_income, total_expenses, f"dashboard_{display_name}_{ai_tone}", user_id), dash_message, user_id) 
