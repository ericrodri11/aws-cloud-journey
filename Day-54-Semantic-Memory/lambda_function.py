...
MEMORY_TABLE_NAME = "FinanceAgent-Memory" # NUEVA TABLA DE MEMORIA
...
memory_table = dynamodb.Table(MEMORY_TABLE_NAME) # CLIENTE DE MEMORIA
...
def get_or_create_monthly_memory(user_id, previous_month_str, previous_month_txs):
    try:
        # 1. Intentar recuperar la memoria existente
        ...
        # 2. Generar nueva memoria si no existe
        prompt = f"""
...
# ==========================================
def invoke_nova_ai(daily_txs, total_daily_spent, monthly_income, monthly_expenses, user_query=None, user_name=DEFAULT_USER_NAME, current_streak=0, historical_memory=""):
    ...
    if user_query:
        ...
        2. Max 2 sentences. Be sharp and ironic. Connect their question to their historical weaknesses if possible.
    ...
    Write ONE sharp sentence celebrating or criticizing their current state. Max 120 chars. Be witty and reference their history.
    """
    ...
    1. <h3><b>Summary:</b></h3> [Sharp critique, max 3 sentences. Compare today with their historical flaws.]
...
def lambda_handler(event, context):
    try:
        ...
        # PRECAUCIÓN: Lógica de caché intacta.
        ...
        # SI NO HAY CACHÉ, ENTONCES LLAMAMOS A LA MEMORIA Y LUEGO A BEDROCK
        if not dash_message:
            # Lógica para obtener el mes anterior (Cálculo robusto)
            today = datetime.datetime.now()
            first_day_this_month = today.replace(day=1)
            last_month = first_day_this_month - datetime.timedelta(days=1)
            previous_month_str = last_month.strftime('%Y-%m')

            # Extraemos solo las transacciones del mes anterior
            previous_month_txs = [t for t in saved_items if t.get('transaction_date', '').startswith(previous_month_str)]
            
            # 🧠 Llamamos al nuevo motor de Memoria
            historical_memory = get_or_create_monthly_memory(user_id, previous_month_str, previous_month_txs)

            if user_query:
                dash_message, email_html_body = invoke_nova_ai(daily_items, total_daily_spent, total_income, total_expenses, user_query, user_name, current_streak, historical_memory)
            else:
                dash_message, email_html_body = invoke_nova_ai(daily_items, total_daily_spent, total_income, total_expenses, None, user_name, current_streak, historical_memory)
