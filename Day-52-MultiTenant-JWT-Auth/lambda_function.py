...
def lambda_handler(event, context):
    try:
        # 1. 🔒 EXTRAER Y DESCIFRAR EL TOKEN JWT DE COGNITO
        headers = event.get('headers', {})
        # Las cabeceras HTTP pueden llegar en mayúscula o minúscula
        ...
        if auth_header.startswith('Bearer '):
            try:
                # Extraemos el cuerpo del JWT (la segunda parte separada por puntos)
                token = auth_header.split(' ')[1]
                payload_b64 = token.split('.')[1]
                # Le añadimos el padding matemático necesario para base64
                payload_b64 += '=' * (-len(payload_b64) % 4) 
                
                # Lo convertimos a un diccionario JSON
                payload = json.loads(base64.b64decode(payload_b64).decode('utf-8'))
                
                # En Cognito, 'sub' es el ID único inmutable del usuario
                user_id = payload.get('sub', DEFAULT_USER_ID)
                user_email = payload.get('email', '')
                
                if user_email:
                    # Usamos lo que hay antes del @ como nombre y lo ponemos en mayúscula (ej: ericridri11 -> Ericridri11)
        ...
        # --- FEATURE FLAGS (PUERTAS TRASERAS) ---
