...
def lambda_handler(event, context):
    try:
        ...
        # Detectamos si es la hora sagrada del reporte (08:00 UTC = 09:00 España Invierno)
        ...
        # Solo lo hacemos si NO es chat Y NO es la hora del reporte.
        # (A la hora del reporte queremos forzar generación completa para tener el HTML largo)
        if not user_query and not is_report_time:
            ...
            if cache_hit:
                dash_message = f"⚡ {cached_message}"
                email_html_body = cached_message 
                from_cache = True
                print(json.dumps({"level": "INFO", "msg": "CACHE HIT - Respuesta servida desde memoria"}))

        # 2. GENERAR RESPUESTA IA (Si no hubo caché o es hora de reporte)
        if not dash_message:
            if user_query:
                # Chat Mode
                dash_message, email_html_body = invoke_nova_ai(daily_items, total_daily_spent, total_income, total_expenses, user_query)
            else:
                # Dashboard / Email Mode (Genera full HTML)
                print(json.dumps({"level": "INFO", "msg": "GENERATING NEW AI ANALYSIS (Report Time or Cache Miss)"}))
                dash_message, email_html_body = invoke_nova_ai(daily_items, total_daily_spent, total_income, total_expenses)
                
                # Guardar en caché SOLO si no es hora de reporte (para no guardar basura en futuras visitas)
                if not is_report_time:
                    cache_key = generate_cache_key(saved_items, total_income, total_expenses, "dashboard")
                    save_to_cache(cache_key, dash_message)
        ...
        if not user_query:
            ...
            if is_report_time: 
                ...
                print("📧 Reporte Matutino (09:00) enviado con HTML completo.")

            # --- 2. ALERTA DE GASTO ALTO (CADA HORA) ---
            if is_alert:
                # [OPCIÓN A: MODO ACTUAL - SMS 📱]
                send_sms_if_needed(total_daily_spent) 

                # [OPCIÓN B: MODO AHORRO - EMAIL]
                # Para activar futuro email de alerta, descomenta abajo:
                # alert_subject = f"🚨 URGENT: Limit Breached ({total_daily_spent:.2f}€)"
                # ses.send_email(...)
