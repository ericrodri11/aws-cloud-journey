...
def invoke_nova_ai(daily_txs, total_daily_spent, monthly_income, monthly_expenses, user_query=None, user_name=DEFAULT_USER_NAME, current_streak=0, historical_memory="", ai_tone="brutal", is_report_time=False):
    ...
    for t in daily_txs:
        ...
        is_inc = any(x in desc for x in ['deposit', 'credit', 'payroll', 'gusto', 'refund', 'united airlines']) or amt < 0
    ...
    try:
        ...
        if is_report_time:
            ...
            {financial_context}
            TODAY'S INCOMES / CREDITS (money that came IN — PRAISE or ignore these): {ai_incomes if ai_incomes else "None"}
            TODAY'S EXPENSES (money that went OUT — CRITIQUE these): {ai_expenses if ai_expenses else "None"}
            {email_tone}
            
            STRICT RULES — VIOLATIONS WILL BE PENALIZED:
            - NEVER criticize items from the INCOME/CREDITS list. They are money received, not spent.
            - ONLY critique items from the EXPENSES list.
            - The "Real daily expenses" in FINANCIAL CONTEXT is the ground truth for how much was spent.
            - If EXPENSES list is empty or small, acknowledge the good behavior.
            - Differentiate between healthy hobbies (sports, education) and reckless spending (casinos, excessive dining).
            - Do not invent generic debts unless the Monthly Surplus is negative.
            
            FORMATTING (Strict HTML):
            1. <h3><b>Summary:</b></h3> [Max 3 sentences. Focus ONLY on expenses, not incomes.]
                ...
                if task in ['daily_report', 'hourly_alert_check']:
                    ...
                    if task == 'daily_report':
                        ...
                        for t in saved_items:
                            ...
                            is_income = any(x in desc for x in ['deposit', 'credit', 'payroll', 'gusto', 'refund', 'united airlines']) or amount < 0
        ...
        if is_scheduled_event or test_email:
            try:
                ...
                if test_email:
                    ...
                    # GET directo garantizado al perfil admin (O(1), nunca se pierde)
                    ...
                    # Escaneo para futuros usuarios adicionales
                    try:
                        from boto3.dynamodb.conditions import Attr
                        response = table.scan(FilterExpression=Attr('transaction_id').eq('METADATA'))
                        for u in response.get('Items', []):
                            u_id = u.get('user_id')
                            if u_id != DEFAULT_USER_ID and not str(u_id).startswith('test_') and u.get('wants_daily_email', True):
                                valid_users.append(u)
                    except Exception as scan_err:
                        logger.warning("Scan for other users failed", extra={"details": str(scan_err)})
                
                # 🔑 DEDUPLICACIÓN: Si existe un segundo perfil creado por login JWT con un
                # sub distinto al DEFAULT_USER_ID, el scan lo añadiría causando doble email.
                # Normalizamos: si algún perfil tiene el mismo email que el admin, lo eliminamos.
                # La forma más robusta es deduplicar por user_id antes de encolar.
                seen_user_ids = set()
                deduped_users = []
                ...
                logger.info(f"Fan-out: {len(valid_users)} profiles found, {len(deduped_users)} after dedup.", extra={"user_ids": list(seen_user_ids)})
                
                for u in deduped_users:
                    sqs.send_message(QueueUrl=SQS_QUEUE_URL, MessageBody=json.dumps({"task": task_name, "user_id": u.get('user_id')}))
                
                logger.info(f"Fan-out complete. Queued {len(deduped_users)} tasks of type '{task_name}'.")
