...
export const VelocityAnalysis = ({ totalExpenses, totalIncome, selectedMonth, selectedYear, prefSavingsGoal }: VelocityAnalysisProps) => {
  ...
  // NUEVO: Calculamos el límite y lo bloqueamos en 0 (Math.max) para evitar límites negativos
  const rawRecommendedBurn = (totalIncome - (parseFloat(prefSavingsGoal) * daysInMonth)) / daysInMonth;
  const recommendedBurn = Math.max(0, rawRecommendedBurn);
  
  const hasIncome = totalIncome > 0;
  const isBurningFast = burnRate > recommendedBurn;
        ...
        <div className="flex justify-between items-end border-b border-gray-100 pb-4">
          ...
          <div className="text-right">
            ...
            <p className={`text-base font-bold ${isBurningFast ? 'text-red-500' : 'text-green-500'}`}>
              {!hasIncome ? 'No income yet' : `${recommendedBurn.toFixed(2)} €`}
            </p>
          </div>
        </div>
        <p className="text-xs text-gray-500 leading-relaxed">
          {!hasIncome
            ? "Waiting for your first deposit to calculate your daily safe limit."
            : isBurningFast
            ? `⚠️ You are spending ${Math.abs(burnRate - recommendedBurn).toFixed(2)}€/day above your safe limit.`
