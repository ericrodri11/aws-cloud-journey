...
def lambda_handler(event, context):
    try:
        ...
        # SMART SYNC: Reloj absoluto del sistema
        current_month = datetime.datetime.now().strftime('%Y-%m')
        has_current_month_data = any(t.get('transaction_date', '').startswith(current_month) for t in saved_items)
        
        needs_sync = force_sync or not saved_items or not has_current_month_data

        if needs_sync:
            logger.info("🔄 Forzando sincronización de datos con la API...")
        ...
        for t in saved_items:
            amt = float(t.get('amount', 0))
            desc = t.get('description', '').lower() # Pasamos a minúscula para la validación
            t_date = t.get('transaction_date', '').split('T')[0].split('#')[0]
            
            if get_accounting_month(t_date, is_income_tx(desc, amt)) == target_month_filter:
                # Filtro espejo del frontend: Ignorar conversiones y transferencias propias
                is_internal = t.get('is_internal', False) or 'to eur' in desc or 'to usd' in desc or 'to gbp' in desc or desc == 'balance' or ('eric fernando' in desc and 'rodriguez' in desc)
