...
def send_sms_if_needed(amount):
    ...
    try:
        ...
        if 'Item' in response:
            print("🔕 Alerta SMS ya enviada hoy. Silencio.")
            return
    except Exception as e:
    ...
    # 2. Enviar SMS Real
    try:
        msg = f"🚨 FINAI ALERT: Gasto alto detectado! Has gastado {amount:.2f}EUR hoy."
        sns.publish(TopicArn=SNS_TOPIC_ARN, Message=msg, Subject="Alerta Gasto")
        print("✅ SMS Enviado.")
        
        # 3. Poner Candado (TTL 24h para no repetir hoy)
        ttl = int(time.time()) + 86400
        # Usamos un user_id genérico para el bloqueo o el mismo USER_ID
        cache_table.put_item(Item={
            'cache_key': lock_key, 
            'status': 'sent', 
            'ttl': ttl,
...
def lambda_handler(event, context):
    try:
        ...
        # Solo enviar email si NO es chat Y NO vino de caché
