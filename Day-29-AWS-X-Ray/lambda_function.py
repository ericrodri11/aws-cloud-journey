...
def invoke_nova_ai(daily_txs, total_daily_spent, monthly_income, monthly_expenses, user_query=None):
    ...
    if user_query:
        ...
        try:
            ...
            response_text = json.loads(resp.get("body").read())['output']['message']['content'][0]['text']
    ...
    try:
        ...
        # Log de Latencia IA
        duration = time.time() - start_time
        log_metric("AILatency", duration, unit="Seconds", properties={"mode": "dashboard"})
        
        return dash_text.strip(), email_text.replace('```html', '').replace('```', '').strip()
        
    except Exception as e:
        print(json.dumps({"level": "ERROR", "msg": "Bedrock Dashboard Error", "details": str(e)}))
...
def generate_html_email(subject, ai_analysis, total_spent, is_alert, transactions):
    ...
    if transactions:
        ...
        <div style="margin-bottom: 24px; margin-top: 30px;">
            <h3 style="color: #94a3b8; font-size: 12px; letter-spacing: 1px; text-transform: uppercase; border-bottom: 1px solid #475569; padding-bottom: 8px; margin-bottom: 10px;">Latest Activity</h3>
            {tx_rows}
        </div>
        """
    else:
