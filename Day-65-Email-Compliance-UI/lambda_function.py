...
FIX incluido en esta versión:
  - COMPLIANCE: Endpoint HTTP GET interceptado para el flujo Unsubscribe.
"""
import json
import base64
import datetime
...
def lambda_handler(event, context):
    try:
            ...
            for record in event['Records']:
                ...
                if task in ['daily_report', 'hourly_alert_check']:
                    ...
                    if task == 'daily_report':
                        ...
                        # FIX: Pasamos el user_id para construir el link de Unsubscribe
                        ses.send_email(
        ...
        # ── COMPLIANCE: GET UNSUBSCRIBE FLOW ──────────────────────────────────
        if http_method == 'GET' and query_params.get('action') == 'unsubscribe':
            ...
            if unsub_user:
                ...
                html_response = """
                <html><body style="font-family:sans-serif;text-align:center;padding:50px;background:#f9fafb;color:#111827;">
                    <div style="background:#fff;border-radius:12px;padding:30px;max-width:400px;margin:0 auto;box-shadow:0 4px 6px rgba(0,0,0,0.05);">
                        <h2 style="color:#16a34a;margin-top:0;">Unsubscribed Successfully</h2>
                        <p style="color:#6b7280;line-height:1.5;">You will no longer receive daily <b>Tough Love</b> emails from the DuroMoney Agent.</p>
                        <p style="font-size:12px;color:#9ca3af;margin-bottom:0;">You can re-enable them anytime from your dashboard settings.</p>
                    </div>
                </body></html>
                """
                return {
