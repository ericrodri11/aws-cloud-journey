...
# ==========================================
def generate_html_email(subject, ai_analysis, total_spent, is_alert, transactions, user_id):
...
# ── Icono por categoría de gasto (Minimalista / Letra inicial CSS) ──
    CATEGORY_COLORS = {
        'housing': '#6366f1', 'bills': '#fb923c', 'groceries': '#10b981',
        'coffee': '#06b6d4', 'food': '#f59e0b', 'transport': '#f97316',
        'travel': '#3b82f6', 'shop': '#ec4899', 'tech': '#8b5cf6',
        'leisure': '#4ade80', 'care': '#f43f5e', 'entertainment': '#a855f7',
        'financial': '#ef4444', 'income': '#16a34a', 'general': '#9ca3af'
    }

    def get_category_style(description, amount):
        d = description.lower()
        if is_income_tx(d, amount): return 'IN', CATEGORY_COLORS['income']
        if any(k in d for k in ['rent', 'mortgage', 'housing']): return 'HO', CATEGORY_COLORS['housing']
        if any(k in d for k in ['market', 'grocery', 'mercadona']): return 'GR', CATEGORY_COLORS['groceries']
        if any(k in d for k in ['bill', 'electric', 'water', 'internet']): return 'BI', CATEGORY_COLORS['bills']
        if any(k in d for k in ['payment', 'credit']): return 'FI', CATEGORY_COLORS['financial']
        if any(k in d for k in ['starbucks', 'coffee']): return 'CF', CATEGORY_COLORS['coffee']
        if any(k in d for k in ['mcdonald', 'burger', 'kfc', 'restaurant', 'food']): return 'FD', CATEGORY_COLORS['food']
        if any(k in d for k in ['uber', 'lyft', 'taxi', 'transit']): return 'TR', CATEGORY_COLORS['transport']
        if any(k in d for k in ['united', 'airline', 'hotel']): return 'TV', CATEGORY_COLORS['travel']
        if any(k in d for k in ['movie', 'cinema', 'ticket']): return 'EN', CATEGORY_COLORS['entertainment']
        if any(k in d for k in ['pharmacy', 'health', 'care']): return 'PC', CATEGORY_COLORS['care']
        if any(k in d for k in ['amazon', 'shop', 'store']): return 'SH', CATEGORY_COLORS['shop']
        if any(k in d for k in ['apple', 'sparkfun', 'electronics', 'netflix', 'spotify']): return 'TC', CATEGORY_COLORS['tech']
        if any(k in d for k in ['gym', 'climb', 'sport']): return 'LS', CATEGORY_COLORS['leisure']
        return 'GN', CATEGORY_COLORS['general']
    ...
    def build_rows(tx_list, is_income_section):
        ...
        for t in tx_list:
            ...
            desc         = t.get('description', '')
            initials, c_color = get_category_style(desc, float(t.get('amount', 0)))
            sign         = '+' if is_income_section else '−'
            amount_color = '#16a34a' if is_income_section else '#111827'
            
            # Hemos reemplazado el emoji por un div con un color de fondo al 15% de opacidad (añadiendo '26' en hex) 
            # y el texto del color puro para dar un look premium.
            rows += f"""
              <tr>
                  ...
                  <table width="100%" cellpadding="0" cellspacing="0" border="0">
                    <tr>
                      <td width="36" style="vertical-align:middle;">
                        <div style="width:32px;height:32px;background-color:{c_color}26;border-radius:8px;
                                    text-align:center;line-height:32px;font-size:12px;font-weight:800;color:{c_color};
                                    letter-spacing:0.5px; font-family:sans-serif;">{initials}</div>
                      </td>
...
def send_sms_if_needed(amount, user_id):
    ...
    try:
        ...
        sns.publish(
            ...
            Message=f"🚨 DuroMoney: You spent {amount:.2f} EUR today. {random.choice(phrases)}",
