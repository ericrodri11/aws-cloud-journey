...
# ==========================================
# PART 4: AI BRAIN (ACTUALIZADO CON MEMORIA)
# ==========================================
def invoke_nova_ai(daily_txs, total_daily_spent, monthly_income, monthly_expenses, user_query=None, user_name=DEFAULT_USER_NAME, current_streak=0, historical_memory="", ai_tone="brutal"):
    ...
    dashboard_prompt = f"""
    You are {user_name}'s financial AI. {tone_instruction} Net Surplus: {net_surplus:.2f}€. Streak: {current_streak} days.
...
def lambda_handler(event, context):
    try:
        ...
        if not dash_message:
            ...
            # --- LA IA RECIBE TU DINERO REAL HOY (total_income), NO EL PROYECTADO ---
            if user_query:
                dash_message, email_html_body = invoke_nova_ai(daily_items, total_daily_spent, total_income, total_expenses, user_query, display_name, current_streak, historical_memory, ai_tone)
            else:
                dash_message, email_html_body = invoke_nova_ai(daily_items, total_daily_spent, total_income, total_expenses, None, display_name, current_streak, historical_memory, ai_tone)
