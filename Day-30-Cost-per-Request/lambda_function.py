...
def calculate_and_log_cost(response_body, mode):
    ...
    try:
        ...
        # Input: $0.00035 per 1,000 tokens | Output: $0.00140 per 1,000 tokens
...
def invoke_nova_ai(daily_txs, total_daily_spent, monthly_income, monthly_expenses, user_query=None):
    ...
    # 2.1 DASHBOARD CALL (Mensaje Corto)
    dashboard_prompt = f"""
    ...
    try:
        ...
        calculate_and_log_cost(body_dash, "dashboard_short") # FinOps Call 1
        ...
        calculate_and_log_cost(body_email, "email_tough_love") # FinOps Call 2
...
def lambda_handler(event, context):
    try:
        ...
        # Solo enviar email si NO es chat
        if not user_query:
