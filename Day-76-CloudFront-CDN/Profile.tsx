...
import { Camera, AlertTriangle, Trash2, CheckCircle2, Save, Mail, TrendingDown, BrainCircuit, LogOut, X } from 'lucide-react';
...
const Profile = ({ 
  ...
  // --- Manejo de la subida de foto a AWS S3 ---
  ...
  return (
    <div className="max-w-3xl mx-auto py-6">
      ...
      <div className="bg-red-50 border border-red-100 rounded-3xl p-8">
        ...
        {showDeleteConfirm ? (
          <div className="bg-white p-5 rounded-2xl border border-red-200 shadow-sm inline-block">
            <p className="text-sm font-bold text-gray-900 mb-4">Are you absolutely sure?</p>
            <div className="flex gap-3">
              <button 
                onClick={handleDeleteAccount}
                disabled={isDeleting}
                className="flex items-center justify-center gap-2 px-5 py-2.5 bg-red-600 hover:bg-red-700 text-white text-sm font-bold rounded-xl transition"
              >
                {isDeleting ? 'Erasing...' : 'Yes, Delete My Account'}
              </button>
              <button 
                onClick={() => setShowDeleteConfirm(false)}
                disabled={isDeleting}
                className="px-5 py-2.5 bg-gray-100 hover:bg-gray-200 text-gray-700 text-sm font-bold rounded-xl transition"
              >
                Cancel
              </button>
            </div>
          ...
          <button 
            ...
            className="flex items-center gap-2 px-5 py-2.5 bg-red-100 hover:bg-red-200 text-red-700 text-sm font-bold rounded-xl transition"
          >
