...
def lambda_handler(event, context):
    try:
        ...
        # ── POST: guardar preferencias (incluyendo data_source y report_email) ─
        ...
        if query_params.get('action') == 'upload_avatar':
            ...
            if image_base64:
                ...
                # Tu URL de CloudFront limpia (CloudFront verá esto como un archivo 100% nuevo)
        ...
        if query_params.get('action') == 'delete_account':
            # 1. Borrar de DynamoDB (Todas las transacciones y perfil)
            table = dynamodb.Table('TU_TABLA_DYNAMODB')  # Cambia por el nombre de tu tabla
            response = table.query(KeyConditionExpression=boto3.dynamodb.conditions.Key('user_id').eq(user_id))
            
            with table.batch_writer() as batch:
                for item in response.get('Items', []):
                    sort_key_name = 'transaction_date' if 'transaction_date' in item else 'sk'
                    batch.delete_item(
                        Key={
                            'user_id': user_id,
                            sort_key_name: item.get(sort_key_name) or item.get('transaction_date')
                        }
                    )
            
            # 2. Borrar de Cognito
            cognito_sub = payload.get('sub')  # Extraído del JWT
            if cognito_sub:
                try:
            ...
            return {
                'statusCode': 200,
                'headers': {"Content-Type": "application/json"},
                'body': json.dumps({"status": "success", "message": "Account annihilated"})
            }
