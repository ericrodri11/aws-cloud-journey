...
def calculate_financial_score(income, expenses):
    ...
    # 1. LISTA CORTA (EMOJIS) - RECUPERADA
    short_reasons = []
    
    # 2. LISTA LARGA (AUDIT LOG)
    ...
    # RETORNAMOS 4 VALORES (Score, Emojis, Audit, Feedback)
...
# PART 2: AI BRAIN
# ==========================================
def invoke_nova_ai(daily_txs, total_daily_spent, monthly_income, monthly_expenses):
    # Cálculo del Superávit (Surplus)
    net_surplus = monthly_income - monthly_expenses
    
    # 1. DASHBOARD PROMPT (Corregido para hablar de Surplus Real)
    dashboard_prompt = f"""
    ...
    # 2. EMAIL PROMPT (Tough Love - Día 24)
    ...
    try:
        ...
        for t in saved_items:
            if t['transaction_date'].startswith(current_month_str):
                ...
                is_income_keyword = any(x in desc for x in ['deposit', 'credit', 'payroll', 'gusto', 'refund'])
                is_income = is_income_keyword or amount < 0
                if is_income:
        ...
        # SCORING (Ahora recoge 4 valores: score, short, audit, feedback)
        ...
        response_payload = {
            ...
            "data": {
                ...
                "score_short_reasons": score_short_reasons, # <--- Emojis recuperados
