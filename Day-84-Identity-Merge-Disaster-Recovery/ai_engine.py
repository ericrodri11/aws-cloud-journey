...
from config import (
    ...
    is_income_tx, is_credit_tx, is_expense_tx
)
...
def invoke_nova_ai(
    ...
    """
    tone_instruction = "Be sharp, ruthless and sarcastic. No sugar-coating, but every criticism must be backed by the provided numbers."
    if ai_tone == "polite":
    ...
    for t in daily_txs:
        ...
        if is_credit_tx(desc, amt):
            ai_incomes_daily.append(tx_data)
        elif is_expense_tx(desc, amt, t):
    ...
    if user_query:
        ...
        prompt = f"""You are directly addressing {user_name} as their financial advisor. {tone_instruction}
{financial_context}
...
RULES: Answer in max 2 sentences. You know the user's name is {user_name}. Use the transaction list above as your source of truth for financial data. If no transactions are listed, say so honestly instead of inventing numbers."""
    ...
    try:
        ...
        if is_report_time:
            email_tone = (
                f"{user_name} spent {actual_daily_expense_total:.2f}€ in real expenses today. Be direct, but keep the critique proportional to the amount and transaction mix."
                if actual_daily_expense_total > 50
                else f"{user_name} spent only {actual_daily_expense_total:.2f}€ today. Acknowledge the controlled spend and focus on one wealth-building action."
            )
...
- Do not call normal small purchases "unacceptable" when Monthly Surplus is positive.
- Do not over-focus on a tiny subscription if larger expenses or higher-impact actions matter more.
- If Monthly Surplus is positive, the advice should prioritize allocation of surplus, automation, emergency fund, or investing over panic-cutting.
- Criticism must be proportional: mention exact expense examples, but avoid dramatic language for low daily spending.
- DUROMONEY TONE: be hard, blunt, and accountability-focused. Do not become soft or generic.
- Numerical consistency is mandatory: if Monthly Surplus is positive, never say expenses are above income.
- If Monthly Surplus is positive but spending is sloppy, say the user has margin but is wasting part of it.
- Use one concrete hard sentence grounded in the expense list, then one constructive action.
