...
def invoke_nova_ai(daily_txs, total_daily_spent, monthly_income, monthly_expenses, user_query=None, user_name=DEFAULT_USER_NAME, current_streak=0, historical_memory="", ai_tone="brutal", is_report_time=False):
    ...
    try:
        ...
        email_text = ""
        if is_report_time:
            email_tone = f"CRITICAL: {user_name} spent {total_daily_spent}€ today." if total_daily_spent > 50 else f"{user_name} spent little today."
            
            email_prompt = f"""
            Act as {user_name}'s financial advisor. {tone_instruction}
            {financial_context}
            Today's Incomes/Refunds (PRAISE THESE): {ai_incomes}
            Today's Expenses (CRITIQUE THESE): {ai_expenses}
            {email_tone}
            
            RULES FOR CRITIQUE:
            - NEVER criticize the Incomes/Refunds list.
            - Differentiate between healthy hobbies (sports, education) and reckless spending (casinos, excessive dining). Hobbies are acceptable IF the Monthly Surplus is high.
            - Do not invent generic debts unless the user's Monthly Surplus is negative.
            
            FORMATTING (Strict HTML):
            1. <h3><b>Summary:</b></h3> [Critique the expenses. Max 3 sentences.]
...
def lambda_handler(event, context):
    try:
        ...
        if is_scheduled_event or test_email:
            try:
                # FIX ARQUITECTÓNICO: Extraemos la hora UTC explícitamente para evitar sustos de zona horaria
                current_hour = datetime.datetime.now(datetime.timezone.utc).hour
                
                # 11 UTC = 13:00 en España (Verano)
                is_report_time = (current_hour == 12) or test_email
                task_name = 'daily_report' if is_report_time else 'hourly_alert_check'
                
                valid_users = []
                
                # FIX ARQUITECTÓNICO: Si es un test, aislamos tu cuenta. Evita correos dobles y ahorra costes.
                if test_email:
