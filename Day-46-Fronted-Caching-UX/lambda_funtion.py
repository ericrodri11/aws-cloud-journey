...
# ==========================================
# PART 0.8: GAMIFICATION ENGINE (NUEVO)
# ==========================================
def update_user_streak(user_id, daily_spent):
    """Maneja el estado de la racha de ahorro en DynamoDB (Single Table Design)."""
    ...
    try:
        ...
        profile = response.get('Item', {
            ...
            'transaction_id': 'METADATA', # Sort Key secundario logico
            'current_streak': 0,
            'highest_streak': 0,
            'last_update': '1970-01-01'
        })
        
        # Evitar doble actualización el mismo día
        ...
        # 2. Lógica de Negocio: Castigo o Recompensa
...
def lambda_handler(event, context):
    try:
        ...
        # Identificar qué hora es primero
        current_hour = datetime.datetime.now().hour
        is_report_time = (current_hour == 7) # 7 UTC = 9:00 España

        today_str = datetime.datetime.now().strftime('%Y-%m-%d')
        yesterday_str = (datetime.datetime.now() - datetime.timedelta(days=1)).strftime('%Y-%m-%d')
        
        # LÓGICA CORREGIDA: Si es el reporte de la mañana, resume ayer. Si no, vigila HOY.
        ...
        if is_report_time:
            ...
            # Si no es la hora de reporte, solo leemos el valor sin actualizar
        ...
        if not user_query:
            if is_report_time: 
                ses.send_email(
                    ...
                    Message={'Subject': {'Data': f"{'🚨' if is_alert else '✅'} Daily Update: {datetime.datetime.now().strftime('%d %b')}"}, 'Body': {'Html': {'Data': generate_html_email(f"Update", email_html_body, total_daily_spent, is_alert, daily_items)}}}
                )
        ...
        return {
            ...
            'body': json.dumps({
                "status": "success",
                "data": {
                    "transactions": saved_items, "dashboard_message": dash_message, "from_cache": from_cache,
                    "financial_score": fin_score, "score_short_reasons": score_short_reasons, "total_income": total_income,
                    "total_expenses": total_expenses, "projected_spend": projected_spend, "current_streak": current_streak # 👈 Expuesto al Frontend
                }
            }, default=str)
        }
    except Exception as e:
